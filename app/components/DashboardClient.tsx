...
import { CheckCircle2, Circle, Clock, Calendar as CalendarIcon, ChevronLeft, ChevronRight, X as CloseIcon, Loader2 } from "lucide-react";
          ...
          (() => {
            ...
            return Array.from(grouped.values()).map(({ regular, twentyMin, title, tag, createdAt }, i) => {
              ...
              if (twentyMin) {
                ...
                const isTogglingThis = togglingIds.has(twentyMin.id);
                // Row is clickable when not toggling and either completed (undo) or countdown expired (complete)
                const isRowClickable = !isTogglingThis && (twentyMin.status || remainingMinutes <= 0);
